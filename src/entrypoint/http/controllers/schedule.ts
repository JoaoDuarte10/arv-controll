...
class ScheduleController {
  ...
  async findAllSchedules(req: Request, res: Response): Promise<Response> {
    ...
    try {
      ...
      return res
        .status(200)
        .json(
          schedules.sort((a, b) => parseFloat(a.time) - parseFloat(b.time)),
        );
    } catch (error) {
  ...
  async findScheduleByDate(req: Request, res: Response): Promise<Response> {
    ...
    try {
      const schedules = await this.scheduleUseCase.findScheduleByDate(
        id_user,
        date,
      );
      return res
  ...
  async saveSchedule(req: Request, res: Response): Promise<Response> {
    ...
    if (
      contact.replace('_', '').toString().length < 16 &&
      contact.replace('_', '').toString().length > 1
    ) {
      return res.status(200).json({
        type: 'error',
  ...
  async deleteSchedule(req: Request, res: Response): Promise<Response> {
    ...
    try {
      await this.scheduleUseCase.deleteSchedule(
        id_user.toString(),
        id.toString(),
      );
      return res.status(200).json({
  ...
  async finishSchedule(req: Request, res: Response): Promise<Response> {
    ...
    try {
      await this.scheduleUseCase.finishSchedule(
        id_user.toString(),
        id.toString(),
      );
      return res.status(200).json({
