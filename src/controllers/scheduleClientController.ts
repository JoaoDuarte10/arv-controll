...
import { ScheduleClientUseCase } from '../useCases/scheduleClientUseCase';
...
class ScheduleClientController {
    ...
    async findAllScheduleClients(req: Request, res: Response): Promise<Response> {
        ...
        try {
            const schedules = await this.scheduleClientUseCase.findAllScheduleClients(id_user);
    ...
    async deleteSchedule(req: Request, res: Response): Promise<Response> {
        ...
        try {
            await this.scheduleClientUseCase.deleteScheduleClients(id_user.toString(), id.toString())
            return res.status(200).json({
                type: 'success',
