...
class ScheduleUseCase {
    ...
    async findScheduleByDate(id_user: string, date: string): Promise<ISchedule[]> {
        try {
            const dateSubtract = moment(new Date).subtract('1', 'day').format('YYYY-MM-DD')
            const findWhereDataLassThen = await this.scheduleRepository.findScheduleWhereDateLassThen(id_user, dateSubtract);
            const findScheduleByDate = await this.scheduleRepository.findScheduleByDate(id_user, date);
    ...
    async saveSchedule({ id_user, client, procedure, date, time, price, phone, pacote, qtdTotalAtendimento }: ISchedule): Promise<void> {
        try {
            ...
            await this.scheduleRepository.saveSchedule({ id_user, client, procedure, date, time, price, phone, pacote, qtdTotalAtendimento })
        } catch (error) {
    ...
    async updateSchedule({ id_user, id, client, procedure, date, time, price, phone, pacote, qtdTotalAtendimento, qtdAtendimento }: ISchedule): Promise<ISchedule> {
        try {
            ...
            const update = await this.scheduleRepository.updateSchedule({ id_user, id, client, procedure, date, time, price, phone, pacote, qtdTotalAtendimento, qtdAtendimento })
            return update
        } catch (error) {
    ...
    async finishSchedule(id_user: string, id: string): Promise<void> {
        ...
        if (qtdAtendimentoAtual >= scheduleAlreadyExists.qtdTotalAtendimento || !scheduleAlreadyExists.pacote) {
            await this.scheduleRepository.deleteSchedule(id_user, id);
