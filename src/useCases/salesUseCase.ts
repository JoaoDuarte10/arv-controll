...
class SalesUseCase {
    ...
    async findSaleByPeriodThatHasClientExists(id_user: string, date1: string, date2: string): Promise<ISales[]> {
        try {
            const find = await this.salesRepository.findSaleByPeriod(id_user, date1, date2);
            return find.filter(item => !!item.client);
        } catch (error) {
            logger.error(`Error SalesUseCase function findSaleByPeriodThatHasClientExists: ${error.message}`)
        }
    }

    async findSaleByClient(id_user: string, client: string): Promise<ISales[]> {
        try {
            const find = await this.salesRepository.findSalesByClient(id_user, client);
            return find
        } catch (error) {
            logger.error(`Error SalesUseCase function findSaleByClient: ${error.message}`)
        }
    }
}
